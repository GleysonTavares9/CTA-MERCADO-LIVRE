import React from 'react';
import { StatusMessage as StatusMessageType } from '../../types';

interface Props {
  status: StatusMessageType;
  className?: string;
}

const getStatusIcon = (type: string) => {
  switch (type) {
    case 'success':
      return (
        <svg className="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
        </svg>
      );
    case 'error':
      return (
        <svg className="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
        </svg>
      );
    default:
      return (
        <svg className="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
        </svg>
      );
  }
};

export const StatusMessage: React.FC<Props> = ({ status, className = '' }) => {
  const getStatusStyles = () => {
    switch (status.type) {
      case 'success':
        return {
          bg: 'bg-green-50/80',
          text: 'text-green-800',
          border: 'border-green-100',
          icon: getStatusIcon('success')
        };
      case 'error':
        return {
          bg: 'bg-red-50/80',
          text: 'text-red-700',
          border: 'border-red-100',
          icon: getStatusIcon('error')
        };
      default:
        return {
          bg: 'bg-blue-50/80',
          text: 'text-blue-800',
          border: 'border-blue-100',
          icon: getStatusIcon('info')
        };
    }
  };

  const styles = getStatusStyles();
  
  if (!status.message) return null;

  return (
    <div 
      className={`relative flex items-start gap-2.5 p-3 rounded-lg border ${styles.bg} ${styles.border} ${styles.text} ${className} text-sm`}
      role="alert"
    >
      <div className="mt-0.5">
        {styles.icon}
      </div>
      <div className="flex-1">
        <p className="leading-tight">{status.message}</p>
      </div>
    </div>
  );
};